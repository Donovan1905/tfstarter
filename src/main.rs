use std::{env, path::PathBuf, fs, io};
...
fn main() {
    ...
    match &cli.cmd {
        ...
        Some(Commands::Get(ttype)) => {
            match ttype.ttype {
                Some(ref _ttype) => {
                    let entries = fs::read_dir(src).expect("Error : Failed to read templates folder")
                        .map(|res| res.map(|e| e.path()))
                        .collect::<Result<Vec<_>, io::Error>>().unwrap();

                    for tpl in &entries {
                        println!("> {}", tpl.into_iter().last().unwrap().to_str().unwrap().bold().purple())
                    }
                }
                None => {
